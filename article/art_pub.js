$(function() {
    ...
    function render(data) {
        var str = '<select name="selectCat" lay-verify="required"><option value="">请选择文章类别</option>';
        for (var i = 0; i < data.length; i++) {
            str += "<option value='" + i + "'>" + data.name + "</option></select>";
        }
        return str;
    }
    ...
    function initSelect() {
        $.ajax({
            ...
            success: function(res) {
                ...
                $('#selectList').empty().html(render(res.data));
                // var strHtm = template('tplSelect', res);
                // // $("#selectList").empty().html(strHtml);
                // $('#selectList').html(strHtm);
            }
        });
    }
    // 封面图片部分js代码
